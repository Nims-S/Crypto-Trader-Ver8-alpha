...
from config.execution import DEFAULT_TOTAL_CAPITAL, DEFAULT_LIVE_INTERVAL_SECONDS, PAPER_TRADING
...
def run_live_cycle(
    ...
    allocations = allocate_capital(strategy_rows, portfolio.total_capital)
...
def run_loop(interval_seconds: int | None = None, total_capital: float | None = None):
    interval = int(interval_seconds or DEFAULT_LIVE_INTERVAL_SECONDS)
    portfolio = PortfolioState(total_capital=total_capital or DEFAULT_TOTAL_CAPITAL, cash=total_capital or DEFAULT_TOTAL_CAPITAL)

    while True:
        result = run_live_cycle(portfolio=portfolio, total_capital=total_capital)
        print(result)
