...
from registry.store import list_strategies, record_experiment, record_evolution_run, upsert_strategy
from research.candidate_generator import mutate_parent
from research.validation import build_walk_forward_folds, default_evolution_window, summarize_walk_forward_reports
...
def _split_mean(wf: dict[str, Any], split_name: str, key: str) -> float:
    split_results = wf.get("split_results") or {}; rows = split_results.get(split_name) or []; vals = []
    for row in rows:
        if isinstance(row, dict) and row.get(key) is not None: vals.append(_safe_float(row.get(key), 0.0))
    return sum(vals) / len(vals) if vals else 0.0
def _score_row(r: dict[str, Any]) -> float:
    wf = (r.get("metrics") or {}).get("walk_forward") or {}
    score = _safe_float(wf.get("score", 0.0), 0.0); pf = _split_mean(wf, "test", "profit_factor"); wr = _split_mean(wf, "test", "win_rate"); trades = _split_mean(wf, "test", "trades"); spread = _safe_float(wf.get("score_spread", 0.0), 0.0)
    bonus = 0.06 * min(pf, 2.0) + 0.04 * wr + 0.02 * min(trades / 20.0, 1.0) - 0.05 * min(spread, 0.5)
    return score + bonus

def _pick_parent(symbol, timeframe):
    s = symbol.lower(); t = timeframe.lower(); rows = list_strategies(active_only=True)
    matches = [r for r in rows if s in {str(x).lower() for x in (r.get("tags") or [])} and t in {str(x).lower() for x in (r.get("tags") or [])}]
    if matches: return sorted(matches, key=_score_row, reverse=True)[0]
...
def _feedback_from_metrics(metrics: dict[str, Any]) -> dict[str, Any]:
    wf = metrics.get("walk_forward") or {}; diag = metrics.get("diagnostics") or {}; activity = diag.get("trade_activity") or {}; means = (wf.get("means") or {}) if isinstance(wf.get("means"), dict) else {}
    return {"top_fail_reasons": wf.get("reasons") or diag.get("top_fail_reasons") or {}, "trade_activity": activity, "mean_test_trades": _safe_float((activity.get("mean") or {}).get("test", 0), 0.0), "mean_val_trades": _safe_float((activity.get("mean") or {}).get("val", 0), 0.0), "mean_train_trades": _safe_float((activity.get("mean") or {}).get("train", 0), 0.0), "mean_test_pf": _safe_float((activity.get("mean_pf") or {}).get("test", 0), 0.0), "mean_val_pf": _safe_float((activity.get("mean_pf") or {}).get("val", 0), 0.0), "mean_train_pf": _safe_float((activity.get("mean_pf") or {}).get("train", 0), 0.0), "mean_test_wr": _safe_float((activity.get("mean_wr") or {}).get("test", 0), 0.0), "mean_val_wr": _safe_float((activity.get("mean_wr") or {}).get("val", 0), 0.0), "mean_train_wr": _safe_float((activity.get("mean_wr") or {}).get("train", 0), 0.0), "score": _safe_float(wf.get("score", 0.0), 0.0), "score_spread": _safe_float(wf.get("score_spread", 0.0), 0.0), "train_mean_score": _safe_float(means.get("train", 0.0), 0.0), "val_mean_score": _safe_float(means.get("val", 0.0), 0.0), "test_mean_score": _safe_float(means.get("test", 0.0), 0.0)}
def _merge_feedback(parent_feedback: dict, store_feedback: dict) -> dict:
    ...
    for key, value in (store_feedback or {}).items():
        if key not in merged or not merged.get(key): merged[key] = value
    return merged
        ...
        for timeframe in timeframes:
            ...
            for child, should_skip, _ in candidate_rows:
                ...
                for fold in wf_folds:
                    st = datetime.fromisoformat(fold.start.replace("Z", "+00:00")); en = datetime.fromisoformat(fold.end.replace("Z", "+00:00")); span = en - st; train_end = st + span * train_ratio; val_end = train_end + span * val_ratio
