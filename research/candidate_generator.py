...
def _apply_directives(params: dict, directives: dict, symbol: str) -> dict:
    ...
    if "min_adx_delta" in directives:
        params["min_adx"] = max(5.0, _safe_float(params.get("min_adx", 10), 10) + _safe_float(directives["min_adx_delta"], 0.0))
    if "min_atr_rank_multiplier" in directives:
        params["min_atr_rank"] = max(0.01, _safe_float(params.get("min_atr_rank", 0.1), 0.1) * _safe_float(directives["min_atr_rank_multiplier"], 1.0))
    if "min_bb_rank_multiplier" in directives:
        params["min_bb_rank"] = max(0.01, _safe_float(params.get("min_bb_rank", 0.1), 0.1) * _safe_float(directives["min_bb_rank_multiplier"], 1.0))
...
def mutate_parent(parent, symbol, timeframe, n_children=4, seed=None, feedback=None, llm_client=None, diversity_pool=None):
    ...
    candidates = []
    seen = set()
    llm_sets = _llm_mutations(base_params, feedback, n_children, llm_client) if llm_client else []
