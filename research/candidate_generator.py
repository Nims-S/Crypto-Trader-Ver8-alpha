...
def _apply_directives(params: dict, directives: dict, symbol: str) -> dict:
    if not directives: return params
    for key in ("use_htf_filter","use_volume_filter","use_structure_filter","use_reclaim_filter","use_trend_filter"):
        if key in directives: params[key] = directives[key]
    if "entry_mode" in directives: params["entry_mode"] = directives["entry_mode"]
    if "min_adx_delta" in directives: params["min_adx"] = max(5.0, _safe_float(params.get("min_adx", 10), 10) + directives["min_adx_delta"])
    if "min_atr_rank_multiplier" in directives: params["min_atr_rank"] = max(0.01, _safe_float(params.get("min_atr_rank", 0.1), 0.1) * directives["min_atr_rank_multiplier"])
    if "min_bb_rank_multiplier" in directives: params["min_bb_rank"] = max(0.01, _safe_float(params.get("min_bb_rank", 0.1), 0.1) * directives["min_bb_rank_multiplier"])
    for key in ("tp1_close_fraction","tp2_close_fraction","tp3_close_fraction","be_trigger_rr","trail_atr_mult","trail_ema20","max_bars_override"):
        if key in directives: params[key] = directives[key]
    if symbol.startswith("BTC") and directives.get("prefer_trend_pullback"): params["use_htf_filter"] = True
    return params
