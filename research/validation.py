...
TRADE_DENSITY_BASE = {"1d": 4, "12h": 5, "8h": 6, "4h": 6, "2h": 8, "1h": 10, "30m": 12, "15m": 14}
...
def build_walk_forward_folds(start: str, end: str, *, folds: int = 3, train_ratio: float = 0.6, val_ratio: float = 0.2, test_ratio: float = 0.2) -> list[WalkForwardSplit]:
    ...
    start_ts = _to_utc_timestamp(start); end_ts = _to_utc_timestamp(end)
    if end_ts <= start_ts: raise ValueError("end must be after start")
    span = end_ts - start_ts
    if span < pd.Timedelta(days=90): raise ValueError("walk-forward range is too short; use at least 90 days")
    train_len = span * train_ratio; val_len = span * val_ratio; test_len = span * test_ratio; step = test_len
...
def _trade_density_threshold(timeframe: str) -> int:
    return TRADE_DENSITY_BASE.get((timeframe or "").strip().lower(), 6)

def _trade_density_score(trades: int, timeframe: str, split_name: str) -> float:
    base = _trade_density_threshold(timeframe)
    target = max(2, int(round(base * (0.75 if split_name == "train" else 0.60))))
    return min(1.0, max(0.0, trades / float(target)))

def _decision_to_dict(decision: Any) -> dict[str, Any]:
    if isinstance(decision, dict): return decision
    try:
    ...
    for key in ("score", "passed", "reasons"):
        if hasattr(decision, key): payload[key] = getattr(decision, key)
    return payload
...
def summarize_walk_forward_reports(fold_reports: list[dict[str, Any]], *, timeframe: str) -> dict[str, Any]:
    ...
    split_decisions = {"train": [], "val": [], "test": []}
    split_results = {"train": [], "val": [], "test": []}
    density_scores = {"train": [], "val": [], "test": []}
    reasons = []
    ...
    passed = (bool(split_scores["val"]) and bool(split_scores["test"]) and val_mean >= 0.55 and test_mean >= 0.55 and min_val_score >= 0.45 and min_test_score >= 0.45 and final_score >= 0.55 and score_spread <= 0.35 and not reasons)
    return {
        "score": round(final_score, 6), "passed": passed, "reasons": reasons, "fold_count": len(fold_reports),
        "composite": round(composite, 6), "stability_penalty": round(stability_penalty, 6), "density_bonus": round(density_bonus, 6), "density_mean": round(density_mean, 6), "score_spread": round(score_spread, 6),
        "means": {"train": round(train_mean, 6), "val": round(val_mean, 6), "test": round(test_mean, 6)},
        "stddev": {"val": round(val_std, 6), "test": round(test_std, 6)},
        "density_scores": {"train": round(density_train, 6), "val": round(density_val, 6), "test": round(density_test, 6)},
        "split_scores": split_scores, "split_decisions": split_decisions, "split_results": split_results,
    }
